...
# Returns the earned run average of a player given the player id
def get_player_stats(player_id):
    player_stats = statsapi.player_stat_data(player_id, group="[pitching]", type="career")
    try:
        player_era = player_stats['stats'][0]['stats']['era']
        return float(player_era)
    
    # Get a new player if they do not have a career era value
    except IndexError:
...
# Compares the era of two players and returns the name of the player with the higher era
def compare_era(playerA, playerB):
    playerA_era = get_player_stats(playerA[0]['id'])
    playerB_era = get_player_stats(playerB[0]['id'])

    if playerA_era > playerB_era:
        return playerA[0]['fullName']
    elif playerB_era > playerA_era:
...
@pitcher_bp.route('/start_game', methods=['GET'])
def start_game():
    """
    Starts a new game by selecting two random players, ensuring they are different.
    Returns the names and eras of both players.
    """
    ...
    playerA_era = get_player_stats(playerA['id'])

    # Generate a new random playerA with a valid career era
    while playerA_era == "Get New Player":
        playerA = get_rand_player()
        playerA_era = get_player_stats(playerA['id'])

    playerB_era = get_player_stats(playerB['id'])

    # Generate a new random player with a valid career era that is NOT the same player as playerA
    while playerB_era == "Get New Player" or playerB == playerA:
        playerB = get_rand_player()
        playerB_era = get_player_stats(playerB['id'])
    ...
    return jsonify({
        ...
        'eraA': f"{playerA_era:.2f}",
        'eraB': f"{playerB_era:.2f}",
    })

@pitcher_bp.route('/player_images', methods=['POST'])
def player_images():
    """
...
@pitcher_bp.route('/check_answer', methods=['POST'])
def check_answer():
    """
    Checks the user's answer for which player has a higher era.
    Updates and returns the streak, whether the answer was correct, and the eras.
    """
    global streak
    data = request.json
    ...
    # Determine the player with the higher era
    higher_player = compare_era(playerA_data, playerB_data)
    correct = False
    ...
    result = {
        'correct': correct,
        'streak': streak,
        'eraA': f"{get_player_stats(playerA_data[0]['id']):.2f}",
        'eraB': f"{get_player_stats(playerB_data[0]['id']):.2f}"
    }
    if not correct:
...
@pitcher_bp.route('/next_question', methods=['POST'])
def next_question():
    """
    Provides a new random player different from the previous one, for the next question.
    Returns the new player and the previous playerA, along with their eras and the current streak.
    """
    data = request.json
    global streak
    ...
    new_eraA = get_player_stats(new_playerA['id'])

    # Ensure the new random player has a valid era and is NOT the same as the previous playerA
    while new_playerA['fullName'] == previous_playerA or new_eraA == "Get New Player":
        new_playerA = get_rand_player()
        new_eraA = get_player_stats(new_playerA['id'])
    
    new_eraB = get_player_stats(previous_playerA_data['id'])
    return jsonify({
        'newPlayerA': new_playerA['fullName'],
        'newPlayerB': previous_playerA,
        'new_eraA': f"{new_eraA:.2f}",
        'new_eraB': f"{new_eraB:.2f}",
        'streak': streak
    })

@pitcher_bp.route('/end_game', methods=['POST'])
def end_game():
    """
