...
from hitter.hitter_routes import hitter_bp, get_rand_player, get_player_image, get_player_stats, compare_hits
import pandas as pd

class TestHitterRoutes(unittest.TestCase):
    def setUp(self):
        self.app = Flask(__name__)
        self.app.register_blueprint(hitter_bp)
    ...
    @patch('hitter.hitter_routes.statsapi.lookup_team')
    @patch('hitter.hitter_routes.team_names', new_callable=dict)
    ...
    def test_index(self):
        with patch('hitter.hitter_routes.render_template') as mock_render_template:
            mock_render_template.return_value = 'Render Content'
            response = self.client.get('/')
            self.assertEqual(response.status_code, 200)
            self.assertIn(b'Render Content', response.data)

    @patch('hitter.hitter_routes.get_rand_player')
    @patch('hitter.hitter_routes.get_player_stats')
    def test_start_game(self, mock_get_player_stats, mock_get_rand_player):
        ...
        self.assertEqual(data['hitsA'], 250)
        self.assertEqual(data['hitsB'], 250)

    @patch('hitter.hitter_routes.statsapi.lookup_player')
    @patch('hitter.hitter_routes.get_player_image')
    ...
    @patch('hitter.hitter_routes.get_player_stats')
    @patch('hitter.hitter_routes.compare_hits')
    @patch('hitter.hitter_routes.statsapi.lookup_player')
    def test_check_answer(self, mock_lookup_player, mock_compare_hits, mock_get_player_stats):
        mock_lookup_player.side_effect = [
            [{'id': 1, 'fullName': 'Player A'}],
            [{'id': 2, 'fullName': 'Player B'}]
        ]
        mock_get_player_stats.side_effect = [300, 250]
        mock_compare_hits.return_value = 'Player A'
        ...
        self.assertEqual(data['hitsA'], '300')
        self.assertEqual(data['hitsB'], '250')
    ...
    def test_next_question(self, mock_get_rand_player, mock_get_player_stats, mock_lookup_player):
        ...
        self.assertEqual(data['new_hitsA'], 300)
        self.assertEqual(data['new_hitsB'], 250)

    @patch('hitter.hitter_routes.streak', new=5)
