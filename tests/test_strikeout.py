...
from strikeout.strikeout_routes import strikeout_bp, get_rand_player, get_player_image, get_player_stats, compare_Ks
import pandas as pd

class TestStrikeoutRoutes(unittest.TestCase):
    def setUp(self):
        self.app = Flask(__name__)
        self.app.register_blueprint(strikeout_bp)
    ...
    @patch('strikeout.strikeout_routes.statsapi.lookup_team')
    @patch('strikeout.strikeout_routes.team_names', new_callable=dict)
    ...
    def test_index(self):
        with patch('strikeout.strikeout_routes.render_template') as mock_render_template:
            mock_render_template.return_value = 'Render Content'
            response = self.client.get('/')
            self.assertEqual(response.status_code, 200)
            self.assertIn(b'Render Content', response.data)

    @patch('strikeout.strikeout_routes.get_rand_player')
    @patch('strikeout.strikeout_routes.get_player_stats')
    def test_start_game(self, mock_get_player_stats, mock_get_rand_player):
        ...
        self.assertEqual(data['ksA'], 250)
        self.assertEqual(data['ksB'], 250)

    @patch('strikeout.strikeout_routes.statsapi.lookup_player')
    @patch('strikeout.strikeout_routes.get_player_image')
    ...
    @patch('strikeout.strikeout_routes.get_player_stats')
    @patch('strikeout.strikeout_routes.compare_Ks')
    @patch('strikeout.strikeout_routes.statsapi.lookup_player')
    def test_check_answer(self, mock_lookup_player, mock_compare_Ks, mock_get_player_stats):
        mock_lookup_player.side_effect = [
            [{'id': 1, 'fullName': 'Player A'}],
            [{'id': 2, 'fullName': 'Player B'}]
        ]
        mock_get_player_stats.side_effect = [300, 250]
        mock_compare_Ks.return_value = 'Player A'
    ...
    def test_next_question(self, mock_get_rand_player, mock_get_player_stats, mock_lookup_player):
        ...
        self.assertEqual(data['new_KsA'], 300)
        self.assertEqual(data['new_KsB'], 250)

    @patch('strikeout.strikeout_routes.streak', new=5)
