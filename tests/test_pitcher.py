...
from pitcher.routes import pitcher_bp, get_rand_player, get_player_image, get_player_stats, compare_era
import pandas as pd

class TestPitcherRoutes(unittest.TestCase):
    def setUp(self):
        self.app = Flask(__name__)
        self.app.register_blueprint(pitcher_bp)
    ...
    @patch('pitcher.routes.statsapi.lookup_team')
    @patch('pitcher.routes.team_names', new_callable=dict)
    ...
    def test_index(self):
        with patch('pitcher.routes.render_template') as mock_render_template:
            mock_render_template.return_value = 'Render Content'
            response = self.client.get('/')
            self.assertEqual(response.status_code, 200)
            self.assertIn(b'Render Content', response.data)

    @patch('pitcher.routes.get_rand_player')
    @patch('pitcher.routes.get_player_stats')
    def test_start_game(self, mock_get_player_stats, mock_get_rand_player):
        mock_get_player_stats.side_effect = [2.50, 2.50]
        ...
        self.assertEqual(data['eraA'], '2.50')
        self.assertEqual(data['eraB'], '2.50')

    @patch('pitcher.routes.statsapi.lookup_player')
    @patch('pitcher.routes.get_player_image')
    ...
    @patch('pitcher.routes.get_player_stats')
    @patch('pitcher.routes.compare_era')
    @patch('pitcher.routes.statsapi.lookup_player')
    def test_check_answer(self, mock_lookup_player, mock_compare_era, mock_get_player_stats):
        mock_lookup_player.side_effect = [
            [{'id': 1, 'fullName': 'Player A'}],
            [{'id': 2, 'fullName': 'Player B'}]
        ]
        mock_get_player_stats.side_effect = [3.00, 2.50]
        mock_compare_era.return_value = 'Player A'
        ...
        self.assertEqual(data['eraA'], '3.00')
        self.assertEqual(data['eraB'], '2.50')
    ...
    def test_next_question(self, mock_get_rand_player, mock_get_player_stats, mock_lookup_player):
        ...
        self.assertEqual(data['new_eraA'], '3.00')
        self.assertEqual(data['new_eraB'], '2.50')

    @patch('pitcher.routes.streak', new=5)
