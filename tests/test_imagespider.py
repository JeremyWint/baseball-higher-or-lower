...
from extract.spiders.imagespider import ImageSpider
...
class TestImageSpider(unittest.TestCase):
    ...
    def test_initialization(self):
        self.assertEqual(self.spider.failed_urls, [])
        self.assertEqual(self.spider.player_data, [])

    @patch('extract.spiders.imagespider.open', new_callable=mock_open, read_data="Team Name,Link\nTeam 1,http://example.com/team1\nTeam 2,http://example.com/team2")
    def test_start_requests(self, mock_file):
        # Simulate file reading by returning lines when the file object is iterated
        mock_file.return_value.__iter__.return_value = iter("Team Name,Link\nTeam 1,http://example.com/team1\nTeam 2,http://example.com/team2".splitlines())
        ...
        mock_file.assert_called_once_with('team_links.csv', 'r')
    ...
    def test_errback(self):
        request = Request(url='http://example.com/fail')
        failure = Exception('Failed request')
        failure.request = request

        self.spider.errback(failure)
        self.assertEqual(len(self.spider.failed_urls), 1)
        self.assertEqual(self.spider.failed_urls[0], 'http://example.com/fail')

    @patch('extract.spiders.imagespider.open', new_callable=mock_open)
    def test_closed(self, mock_file):
        ...
        self.spider.failed_urls = ['http://example.com/fail1', 'http://example.com/fail2']

        self.spider.closed('finished')

        mock_file.assert_any_call('player_image.csv', mode='w', newline='', encoding='utf-8')
        mock_file.assert_any_call('failed_urls.csv', mode='w', newline='', encoding='utf-8')

        handle = mock_file()
        handle.write.assert_any_call('name,team,image_url\r\n')
        ...
        handle.write.assert_any_call('failed_url\r\n')
        handle.write.assert_any_call('http://example.com/fail1\r\n')
        handle.write.assert_any_call('http://example.com/fail2\r\n')
