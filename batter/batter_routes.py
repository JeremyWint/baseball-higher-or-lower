...
def get_player_stats(player_id):
    ...
    # Get a new player if they do not have a career batting average value
    except IndexError:
...
def start_game():
    ...
    # Generate a new random playerA with a valid batting average
    while playerA_avg == "Get New Player":
        playerA = get_rand_player()
        playerA_avg = get_player_stats(playerA['id'])
    
    # Generate a new random playerB with a valid batting average that is NOT the same player as playerA
    playerB_avg = get_player_stats(playerB['id'])
    while playerB_avg == "Get New Player" or playerB == playerA:
        playerB = get_rand_player()
        playerB_avg = get_player_stats(playerB['id'])
...
def next_question():
    ...
    # Ensure the new random player has a valid batting average and is NOT the same player as the previous playerA
    while new_playerA['fullName'] == previous_playerA or new_baA == "Get New Player":  
        new_playerA = get_rand_player()
        new_baA = get_player_stats(new_playerA['id'])
